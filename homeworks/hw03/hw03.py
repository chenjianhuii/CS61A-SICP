...
def g_iter(n):
    ...
    if n < 4:
        ...
        k, one, two, three = 3, 1, 2, 3
        while k < n:
            k, one, two, three = k + 1, two, three, one * 3 + two * 2 + three
        return three
...
def missing_digits(n):
    ...
    n, r = divmod(n, 10)
    last = n % 10
    return missing_digits(n) + max(0, r-last-1)
...
def count_change(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    # We implement in opposite direction for there is no limit to the maximum coin
    def helper(total, min_coin):
        if total == 0:
            return 1
        if total < (1 << min_coin):
            return 0
        with_min_coin = helper(total-(1<<min_coin), min_coin)
        without_min_coin = helper(total, min_coin+1)
...
def make_anonymous_factorial():
    ...
    """
    # Y combinator
    return (lambda f: lambda x: f(f,x))(lambda self, x: 1 if x<=1 else x*self(self, x-1))
