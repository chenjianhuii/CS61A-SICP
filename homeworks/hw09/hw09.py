...
def interleaved_sum(n, odd_term, even_term):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(x, acc, cur):
        if x == n + 1:
            return acc
        elif cur:
            return helper(x+1, acc+odd_term(x), 1-cur)
        else:
            return helper(x+1, acc+even_term(x), 1-cur)
    return helper(1, 0, 1)
...
def mutate_reverse(link):
    ...
    #     second = third
    # link = first
    # We can't change the link binding inside functions, thus we change the val stored in each node
    list_val = []
    tmp = link
    while tmp:
        list_val.append(tmp.first)
        tmp = tmp.rest
