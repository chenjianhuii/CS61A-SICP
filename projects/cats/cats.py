...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def func(paragraph):
        processed = split(remove_punctuation(lower(paragraph)))
        for word in topic:
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words: 
        ...
        for w in valid_words:
            temp = diff_function(user_word, w, limit)
            if temp < min_val and temp <= limit:
                min_val = temp
                ret = w
    return ret
    # END PROBLEM 5
...
def shifty_shifts(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def helper(start, goal, cur, limit):
        if cur > limit:
            return cur
        if not (start and goal):
            return cur + max(len(start), len(goal))
        if start[0] == goal[0]:
            return helper(start[1:], goal[1:], cur, limit)
        return helper(start[1:], goal[1:], cur+1, limit)
...
def meowstake_matches(start, goal, limit):
    ...
    if limit < 0:
        return float('inf')
    elif not (start and goal): # Fill in the condition
...
def time_per_word(times_per_player, words):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    return game(words, [[times_per_player[j][i+1]-times_per_player[j][i] for i in range(len(words))]\
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fastest = [min(list(players), key=lambda p: time(game, p, w)) for w in words]
    result = [[] for _ in players]
    word_list = all_words(game)
    for w, p in enumerate(fastest):
        result[p].append(word_list[w])
    return result
    # END PROBLEM 10
...
def faster_autocorrect(user_word, valid_words, diff_function, limit):
    ...
    # BEGIN PROBLEM EC2
    "*** YOUR CODE HERE ***"
    if (user_word, diff_function, limit) in local_cache:
        return local_cache[(user_word, diff_function, limit)]
    if user_word in valid_words: 
        ret = user_word
    else:
        ret = user_word
        ...
        for w in valid_words:
            ...
            if temp < min_val and temp <= limit:
                min_val = temp
                ret = w
# Use the following statement need one more call to diff_function which will result in the failure of assertion
        # min_word = min(valid_words, key=lambda x: diff_function(user_word, x, limit)) 
        # min/max will return the first element with minimum/maximum value
    local_cache[(user_word, diff_function, limit)] = ret
    return ret
    # END PROBLEM EC2
