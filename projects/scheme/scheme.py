...
from ast import arg
from signal import raise_signal  # Python 2 compatibility
...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        except (TypeError, SchemeError) as e:
            raise SchemeError(e.__str__())
        rest = rest.map(lambda expr: scheme_eval(expr, env))
        return complete_apply(first, rest, env)
        # END PROBLEM 4
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('The arguments don\'t match the formal parameters')
        child_frame = Frame(self)
        while not (formals is nil or vals is nil):
...
def do_and_form(expressions, env):
    ...
    if is_false_primitive(first_val) or expressions.rest is nil:
        return first_val
    else:
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False
    first_val = scheme_eval(expressions.first, env)
    if is_true_primitive(first_val):
        return first_val
    else:
...
class MuProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        """Make a frame that binds my formal parameters to ARGS, a Scheme list
        of values, for a dynamic-scoped call evaluated in ENV environment."""
