...
def prune_min(t):
    ...
    "*** YOUR CODE HERE ***"
    if len(t.branches) == 2:
        new_branches = [min(t.branches, key=lambda t: t.label)]
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        possibilities.append((f'+[{code_char}]'+match, cost+1))
        # Delete
        match, cost = helper_align(skeleton_idx+1, code_idx)
        possibilities.append((f'-[{skel_char}]'+match, cost+1))
...
def insert(link, value, index):
    ...
    if link is not Link.empty and index == 0:
        old_val = link.first
        link.first = value
        link.rest = Link(old_val, link.rest)
